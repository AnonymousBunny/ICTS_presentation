...
\begin{frame}
	\begin{itemize}
		\item<1-> A result of Kremer \footnote{The result of Kremer only lower bounds clean-quantum communication complexity.[BdW01] observed that it readily implies general quantum communication lower bounds too. Another proof was given by Lee and Shraibman [LS07]}: 
...
\begin{frame}
	...
	\begin{itemize}
		...
		$$ \log \text{spar}(g) \leq \text{poly}(\log ( \text{rank}^{1/3} (g \circ \text{AND}_2), \log(n) )) $$
