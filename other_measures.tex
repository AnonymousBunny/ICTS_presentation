...
\begin{frame}{prior results}
	\begin{itemize}
		...
		\item<2-> Thus, log approximate rank conjecture for a class of functions also implies log equivalence conjecture for that class of functions.
...
\begin{frame}{prior results}
	...
	\begin{itemize}
		...
		\textbf{Note: }This is not the Fourier decomposition where the domain is $\{ \pm 1\}^n$. Also note that we are working over $\mathbb{R}$, not $\mathbb{F}_2$. 
	\end{itemize}
...
\begin{frame}{}
	...
	\begin{itemize}
		...
		\item<2-> Our main result\footnote{Other direction is trivial}:
		
		$$ \log \text{spar}(g) \leq \text{poly}\left(\log ( \text{rank}^{1/3} (g \circ \text{AND}_2), \log(n) )\right) $$ 
	\end{itemize}
...
\textbf{How this implies the Log Equivalence Conjecture}
	
	\begin{itemize}
		\item<1-> Recall that we want to upper bound $D(g)$ in terms of $\text{log} \left( \text{rank}^{1/3} (g \circ \text{AND}_2)\right)$
